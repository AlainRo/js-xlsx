...
function namesValues(){
    ...
    var sheets = wb.SheetNames; //.map(e => e.replace(/\s+/g, '_'));
    //console.log(sheets);
    //sheets.map(printAscii);
    names.map(n => {//console.log(n.Ref);
        var ref = n.Ref.split('!');
        var ws, rg, wsn;
        if (ref.length >1){
            rg = X.utils.decode_range(ref[1]);
            wsn = ref[0].replace(/\'/g, ''); // RETIRE LES ' POSSIBLES
            //console.log(ref[0]);
